...
class Pdf417EncoderTest {
    ...
    @Test
    fun testBasicPdf417() {
        val matrix = QroseEncoders.PDF417().encode("Hello, PDF417!")
    ...
    fun testCompactPdf417() {
        val standard = QroseEncoders.PDF417(compact = false).encode("Compact Test")
        val compact = QroseEncoders.PDF417(compact = true).encode("Compact Test")
    ...
    fun testCompactionModes() {
        // Text mode
        val textMatrix = QroseEncoders
            .PDF417(compaction = Pdf417Compaction.Text)
            .encode("UPPER lower 12345!.,-")
        assertTrue(textMatrix.width > 0)

        // Numeric mode
        val numMatrix = QroseEncoders
            .PDF417(compaction = Pdf417Compaction.Numeric)
            .encode("012345678901234567890123456789")

        assertTrue(numMatrix.width > 0)

        // Byte mode with binary data
        val byteMatrix = QroseEncoders
            .PDF417(compaction = Pdf417Compaction.Byte)
            .encode("Binary\u0000\u0001\u0002Payload")
        assertTrue(byteMatrix.width > 0)

        // Auto mode with UTF-8 non-ASCII characters
        val utf8Matrix = QroseEncoders
            .PDF417()
            .encode("Привет, мир! PDF417 2D barcode")
        assertTrue(utf8Matrix.width > 0)
    }

    @Test
    fun testCustomDimensions() {
        val matrix = QroseEncoders
            .PDF417(dimensions = Pdf417Dimensions(minCols = 4, maxCols = 4, minRows = 3, maxRows = 20))
            .encode("Fixed 4 columns test")
    ...
    fun testErrorCorrectionLevels() {
        val m0 = QroseEncoders
            .PDF417(errorCorrectionLevel = Pdf417ErrorCorrectionLevel.Level0)
            .encode("EC Test")

        val m4 = QroseEncoders
            .PDF417(errorCorrectionLevel = Pdf417ErrorCorrectionLevel.Level4)
            .encode("EC Test")
