...
internal class Code128Encoder(
    val compact: Boolean = true,
    val codeSet: Code128Type? = null
) : BarcodeEncoder {
    ...
    override fun encode(data: String): BooleanArray {
        val forcedCodeSet = check(data, codeSet)
        ...
        private var minPath: Array<Array<Code128Latch?>>? = null

        fun encodeCompact(contents: String): BooleanArray {
            ...
            encode(contents, Code128Charset.NONE, 0)
            ...
            var charset = Code128Charset.NONE
            var i = 0
            while (i < length) {
                val latch = minPath!![charset.ordinal][i]
                when (latch) {
                    Code128Latch.A -> {
                        charset = Code128Charset.A
                        addPattern(
                            patterns,
                    ...
                    Code128Latch.SHIFT -> addPattern(patterns, CODE_SHIFT, checkSum, checkWeight, i)
                    else -> {}
                }
                if (charset == Code128Charset.C) {
                    ...
                    patternIndex =
                        when (contents[i]) {
                            ...
                            ESCAPE_FNC_4 -> if (charset == Code128Charset.A && latch != Code128Latch.SHIFT ||
                                charset == Code128Charset.B && latch == Code128Latch.SHIFT
                            ) {
                                CODE_FNC_4_A
                            } else {
                                CODE_FNC_4_B
                    ...
                    if ((charset == Code128Charset.A && latch != Code128Latch.SHIFT ||
                                charset == Code128Charset.B && latch == Code128Latch.SHIFT) &&
                        patternIndex < 0
                    ) {
                        patternIndex += '`'.code
        ...
        private fun canEncode(contents: CharSequence, charset: Code128Charset, position: Int): Boolean {
            val c = contents[position]
            return when (charset) {
                Code128Charset.A -> c == ESCAPE_FNC_1 || c == ESCAPE_FNC_2 || c == ESCAPE_FNC_3 || c == ESCAPE_FNC_4 || A.indexOf(
                    c
                ) >= 0

                Code128Charset.B -> c == ESCAPE_FNC_1 || c == ESCAPE_FNC_2 || c == ESCAPE_FNC_3 || c == ESCAPE_FNC_4 || B.indexOf(
                    c
                ) >= 0

                Code128Charset.C -> c == ESCAPE_FNC_1 || position + 1 < contents.length &&
        ...
        /**
         * Encode the string starting at position starting with the character set charset
         */
        private fun encode(contents: CharSequence, charset: Code128Charset, position: Int): Int {
            ...
            var minLatch: Code128Latch? = Code128Latch.NONE
            val atEnd = position + 1 >= contents.length
            val sets = arrayOf(Code128Charset.A, Code128Charset.B)
            for (i in 0..1) {
                if (canEncode(contents, sets[i], position)) {
                    var cost = 1
                    var latch = Code128Latch.NONE
                    if (charset != sets[i]) {
                        cost++
                        latch = Code128Latch.valueOf(
                            sets[i].toString()
                        )
                    }
                    if (!atEnd) {
            ...
            if (canEncode(contents, Code128Charset.C, position)) {
                var cost = 1
                var latch = Code128Latch.NONE
                if (charset != Code128Charset.C) {
                    cost++
                    latch = Code128Latch.C
                }
                val advance = if (contents[position] == ESCAPE_FNC_1) 1 else 2
                if (position + advance < contents.length) {
                    cost += encode(contents, Code128Charset.C, position + advance)
                }
                if (cost < minCost) {
                    minCost = cost
                    minLatch = latch
...
private enum class Code128Charset {
    A,
    B,
    C,
    NONE
...
private enum class Code128Latch {
    A,
    B,
    C,
    SHIFT,
